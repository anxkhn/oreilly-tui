...
{nav_items}
    ...
    def _build_nav_points(
        ...
        for item in toc_items:
            nav_id = item.get("fragment") or item.get("ourn", "").split(":")[
                -1
            ].replace(".html", "")
            label = html.escape(item.get("title", ""))
            href = (
                item.get("reference_id", "").split("-/")[-1]
            ...
            if children:
                child_points, play_order = self._build_nav_points(
                    children, play_order, indent + 2
                )
